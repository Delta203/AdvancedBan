  ...
  <div class="row">
    <div class="col-md-8">
      <div class="bg-body-tertiary py-5 p-5 mb-5">
        <h1><?php echo($messages_banlist) ?></h1>
        <table class="table table-striped mt-3">
          ...
          <tbody>
            <?php
            $sql = "SELECT Reason, PlayerUUID FROM AB_Bans LIMIT $max_banlist OFFSET $offset";
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-center">
          <div class="btn-group me-2" role="group">
            <?php
            $pages = getGlobaMutesCount() / $max_banlist;
            for ($i = 1; $i <= $pages+1; $i++) { ?>
              <a class="btn btn-outline-secondary <?php if ($i == $page) echo("active"); ?>"
                href="<?php echo($root); ?>/?p=banlist&page=<?php echo($i); ?>"><?php echo($i); ?></a>
            <?php } ?>
          </div>
        </div>
      </div>
